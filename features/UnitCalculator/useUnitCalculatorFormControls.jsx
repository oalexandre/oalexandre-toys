...
export const useUnitCalculatorFormControls = () => {
  ...
  useEffect(() => {
    ...
    if (typeQuery) {
      const level = measurements.find(
        item => item.type.startsWith(typeQuery) || parseInt(typeQuery) === item.id
      );
      ...
      if (levelType && unitQuery) {
        unitTemp = levelUnits
          .find(
            item =>
              item.label.toLowerCase().startsWith(unitQuery) ||
              item.short === unitQuery ||
              unitQuery === item.id
          )
          ?.unit.toString();
        ...
        // resolves conflicts
        unitTemp =
          levelType === "length" && unitQuery === "m"
            ? "meters"
            : (levelType === "length" && unitQuery === "mi") ||
              (levelType === "length" && unitQuery === "mil")
            ? "miles"
            : unitTemp;
      }
